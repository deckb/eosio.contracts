...
inline constexpr int64_t charge_frac  = 1'000'000'000'000'000ll; // 1.0 = 10^15
inline constexpr int64_t stake_weight = 100'000'000'0000ll; // 10^12

struct charge_config_resource {
...
FC_REFLECT(charge_config_resource,                                                             //
...
struct charge_config {
   charge_config_resource net          = {};
   charge_config_resource cpu          = {};
   fc::optional<uint32_t> rent_days    = {};
   fc::optional<asset>    min_rent_fee = {};
};
FC_REFLECT(charge_config, (net)(cpu)(rent_days)(min_rent_fee))

struct charge_state_resource {
   uint8_t        version;
   int64_t        weight;
...
FC_REFLECT(charge_state_resource,                                                                           //
...
struct charge_state {
   uint8_t               version;
   charge_state_resource net;
   charge_state_resource cpu;
   uint32_t              rent_days;
   asset                 min_rent_fee;
};
FC_REFLECT(charge_state, (version)(net)(cpu)(rent_days)(min_rent_fee))

using namespace eosio_system;

struct charge_tester : eosio_system_tester {

   charge_tester() { create_accounts_with_resources({ N(eosio.reserv) }); }
   ...
   template <typename F>
   charge_config make_config(F f) {
      charge_config config;

      config.net.current_weight_ratio = charge_frac;
      config.net.target_weight_ratio  = charge_frac / 100;
      ...
      config.cpu.current_weight_ratio = charge_frac;
      config.cpu.target_weight_ratio  = charge_frac / 100;
   ...
   charge_config make_config() {
      return make_config([](auto&) {});
   }

   template <typename F>
   charge_config make_default_config(F f) {
      charge_config config;
      f(config);
      return config;
   }

   action_result configbw(const charge_config& config) {
      ...
      auto resource_conf_vo = [&optional_to_variant](const charge_config_resource& c ) {
      ...
      return push_action(config::system_account_name, N(configcharge), mvo()("args", std::move(conf)));

      // If abi_serializer worked correctly, the following is all that would be needed:
      //return push_action(config::system_account_name, N(configcharge), mvo()("args", config));
   }

   action_result chargeexec(name user, uint16_t max) {
      return push_action(user, N(chargeexec), mvo()("user", user)("max", max));
   }

   action_result charge(const name& payer, const name& receiver, uint32_t days, int64_t net_frac, int64_t cpu_frac,
                        const asset& max_payment) {
      return push_action(payer, N(charge),
                         mvo()("payer", payer)("receiver", receiver)("days", days)("net_frac", net_frac)(
                               "cpu_frac", cpu_frac)("max_payment", max_payment));
   }

   charge_state get_state() {
      vector<char> data = get_row_by_account(config::system_account_name, {}, N(rent.state), N(rent.state));
      return fc::raw::unpack<charge_state>(data);
   }
   ...
   void check_charge(const name& payer, const name& receiver, uint32_t days, int64_t net_frac, int64_t cpu_frac,
      ...
      BOOST_REQUIRE_EQUAL("", charge(payer, receiver, days, net_frac, cpu_frac, expected_fee));
      ...
      if (false) {
         ...
         ilog("before_state.net.weight_ratio:            ${x}",
              ("x", before_state.net.weight_ratio / double(charge_frac)));
         ilog("before_state.net.assumed_stake_weight:    ${x}", ("x", before_state.net.assumed_stake_weight));
...
BOOST_AUTO_TEST_SUITE(eosio_system_charge_tests)

BOOST_FIXTURE_TEST_CASE(config_tests, charge_tester) try {
   BOOST_REQUIRE_EQUAL("missing authority of eosio",
                       push_action(N(alice1111111), N(configcharge), mvo()("args", make_config())));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("charge hasn't been initialized"), chargeexec(N(alice1111111), 10));
   ...
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("current_weight_ratio is too large"),
                       configbw(make_config([](auto& c) { c.net.current_weight_ratio = charge_frac + 1; })));
   ...
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("weight can't grow over time"),
                       configbw(make_config([](auto& c) { c.net.target_weight_ratio = charge_frac + 1; })));
   ...
   // cpu assertions
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("current_weight_ratio is too large"),
                       configbw(make_config([](auto& c) { c.cpu.current_weight_ratio = charge_frac + 1; })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("assumed_stake_weight/target_weight_ratio is too large"),
                       configbw(make_config([](auto& c) {
   ...
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("weight can't grow over time"),
                       configbw(make_config([](auto& c) { c.cpu.target_weight_ratio = charge_frac + 1; })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("assumed_stake_weight does not have a default value"),
...
BOOST_FIXTURE_TEST_CASE(weight_tests, charge_tester) try {
   produce_block();

   auto net_start  = (charge_frac * 11) / 100;
   auto net_target = (charge_frac * 1) / 100;
   auto cpu_start  = (charge_frac * 11) / 1000;
   auto cpu_target = (charge_frac * 1) / 1000;

   BOOST_REQUIRE_EQUAL("", configbw(make_config([&](charge_config& config) {
   ...
   auto check_weight = [&] {
      ...
      BOOST_REQUIRE(near(           //
            ...
            int64_t(state.net.assumed_stake_weight * eosio::chain::int128_t(charge_frac) /
   ...
   for (int i = 0; i <= 6; ++i) {
      if (i == 2) {
         ...
         produce_block(fc::days(1) - fc::milliseconds(500));
         BOOST_REQUIRE_EQUAL("", chargeexec(config::system_account_name, 10));
      }
   ...
   // Extend transition time
   {
      int i = 7;
      produce_block(fc::days(1) - fc::milliseconds(500));
      BOOST_REQUIRE_EQUAL("", configbw(make_default_config([&](charge_config& config) {
...
BOOST_AUTO_TEST_CASE(rent_tests) try {
   {
      ...
      BOOST_REQUIRE_EQUAL(t.wasm_assert_msg("charge hasn't been initialized"), //
                          t.charge(N(bob111111111), N(alice1111111), 30, charge_frac / 4, charge_frac / 8,
                                   asset::from_string("1.000 TST")));

      BOOST_REQUIRE_EQUAL("", t.configbw(t.make_config([&](auto& config) {
         config.net.current_weight_ratio = charge_frac;
         config.net.target_weight_ratio  = charge_frac;
         config.net.assumed_stake_weight = stake_weight;
         ...
         config.cpu.current_weight_ratio = charge_frac;
         config.cpu.target_weight_ratio  = charge_frac;
         config.cpu.assumed_stake_weight = stake_weight;
      ...
      BOOST_REQUIRE_EQUAL(
            ...
            t.charge(N(bob111111111), N(alice1111111), 30, charge_frac, charge_frac, asset::from_string("1.000 TST")));
      BOOST_REQUIRE_EQUAL(
            t.wasm_assert_msg("market doesn't have resources available"), //
            t.charge(N(bob111111111), N(alice1111111), 30, 0, charge_frac, asset::from_string("1.0000 TST")));
      BOOST_REQUIRE_EQUAL(
            t.wasm_assert_msg("market doesn't have resources available"), //
            t.charge(N(bob111111111), N(alice1111111), 30, charge_frac, 0, asset::from_string("1.0000 TST")));

      BOOST_REQUIRE_EQUAL("", t.configbw(t.make_default_config([&](auto& config) {
         // weight = stake_weight
         config.net.current_weight_ratio = charge_frac/2;
         config.net.target_weight_ratio  = charge_frac/2;

         // weight = stake_weight
         config.cpu.current_weight_ratio = charge_frac/2;
         config.cpu.target_weight_ratio  = charge_frac/2;
      })));
      ...
      t.check_charge(N(aaaaaaaaaaaa), N(aaaaaaaaaaaa), 30, charge_frac * .1, charge_frac * .2,
                     asset::from_string("300000.0000 TST"), net_weight * .1, cpu_weight * .2);

      // Start decay
      t.produce_block(fc::days(30) - fc::milliseconds(500));
      BOOST_REQUIRE_EQUAL("", t.chargeexec(config::system_account_name, 10));
      ...
      t.check_charge(N(aaaaaaaaaaaa), N(aaaaaaaaaaaa), 30, charge_frac * .02, charge_frac * .02,
   ...
   auto init = [](auto& t, bool rex) {
      ...
      BOOST_REQUIRE_EQUAL("", t.configbw(t.make_config([&](auto& config) {
         ...
         config.net.current_weight_ratio = charge_frac / 4;
         config.net.target_weight_ratio  = charge_frac / 4;
         config.net.assumed_stake_weight = stake_weight;
         config.net.exponent             = 2;
         config.net.max_price            = asset::from_string("2000000.0000 TST");

         // weight = stake_weight * 4 / 2
         config.cpu.current_weight_ratio = charge_frac / 5;
         config.cpu.target_weight_ratio  = charge_frac / 5;
...

   {
      ...
      BOOST_REQUIRE_EQUAL(
            ...
            t.charge(N(bob111111111), N(alice1111111), 20, charge_frac, charge_frac, asset::from_string("1.0000 TST")));
      BOOST_REQUIRE_EQUAL(                                   //
            t.wasm_assert_msg("net_frac can't be negative"), //
            t.charge(N(bob111111111), N(alice1111111), 30, -charge_frac, charge_frac,
      ...
      BOOST_REQUIRE_EQUAL(                                    //
            ...
            t.charge(N(bob111111111), N(alice1111111), 30, charge_frac + 1, charge_frac,
                     asset::from_string("1.0000 TST")));
      BOOST_REQUIRE_EQUAL(                                    //
            t.wasm_assert_msg("cpu can't be more than 100%"), //
            t.charge(N(bob111111111), N(alice1111111), 30, charge_frac, charge_frac + 1,
                     asset::from_string("1.0000 TST")));
      BOOST_REQUIRE_EQUAL(
            t.wasm_assert_msg("max_payment is less than calculated fee: 3000000.0000 TST"), //
            t.charge(N(bob111111111), N(alice1111111), 30, charge_frac, charge_frac, asset::from_string("1.0000 TST")));
   ...
   // net:100%, cpu:100%
   {
      ...
      BOOST_REQUIRE_EQUAL(
            ...
            t.charge(N(aaaaaaaaaaaa), N(bbbbbbbbbbbb), 30, 10, 10, asset::from_string("3000000.0000 TST")));
      t.check_charge(N(aaaaaaaaaaaa), N(bbbbbbbbbbbb), 30, charge_frac, charge_frac,
      ...
      BOOST_REQUIRE_EQUAL( //
            ...
            t.configbw(t.make_default_config([&](auto& config) {
               config.net.current_weight_ratio = charge_frac / 4 + 1;
               config.net.target_weight_ratio  = charge_frac / 4 + 1;
               config.cpu.current_weight_ratio = charge_frac / 5;
               config.cpu.target_weight_ratio  = charge_frac / 5;
            })));
      BOOST_REQUIRE_EQUAL( //
            ...
            t.configbw(t.make_default_config([&](auto& config) {
               config.net.current_weight_ratio = charge_frac / 4;
               config.net.target_weight_ratio  = charge_frac / 4;
               config.cpu.current_weight_ratio = charge_frac / 5 + 1;
               config.cpu.target_weight_ratio  = charge_frac / 5 + 1;
            })));
      BOOST_REQUIRE_EQUAL( //
            "",            //
            t.configbw(t.make_default_config([&](auto& config) {
   ...
   // net:30%, cpu:40%, then net:5%, cpu:10%
   {
      ...
      t.check_charge(N(aaaaaaaaaaaa), N(bbbbbbbbbbbb), 30, charge_frac * .3, charge_frac * .4,
      ...
      t.check_charge(N(aaaaaaaaaaaa), N(bbbbbbbbbbbb), 30, charge_frac * .05, charge_frac * .10,
   ...
   // net:50%, cpu:50% (but with non-zero min_price and also an exponent of 2 to simplify the math)
   {
      ...
      t.check_charge(N(aaaaaaaaaaaa), N(bbbbbbbbbbbb), 30, charge_frac * .5, charge_frac * .5,
...

   {
      ...
      BOOST_REQUIRE_EQUAL(t.wasm_assert_msg("market doesn't have enough resources available"), //
                          t.charge(N(bob111111111), N(alice1111111), 30, charge_frac / 1000, charge_frac / 1000,
                                   asset::from_string("1.0000 TST")));
...

   {
      ...
      t.check_charge(N(aaaaaaaaaaaa), N(bbbbbbbbbbbb), 30, charge_frac * .1, charge_frac * .2,
      ...
      t.check_charge(N(aaaaaaaaaaaa), N(bbbbbbbbbbbb), 30, charge_frac * .2, charge_frac * .2,
