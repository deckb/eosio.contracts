...
struct powerup_tester : eosio_system_tester {
   ...
   action_result configpowerup(const powerup_config& config) {
      ...
      auto conf = mvo("net",          resource_conf_vo(config.net))
                     ...
                     ("powerup_days",    optional_to_variant(config.rent_days))
                     ("min_powerup_fee", optional_to_variant(config.min_rent_fee))
      ;
...
BOOST_FIXTURE_TEST_CASE(config_tests, powerup_tester) try {
   ...
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("rent_days must be > 0"),
                       configpowerup(make_config([&](auto& c) { c.rent_days = 0; })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("min_rent_fee doesn't match core symbol"), configpowerup(make_config([&](auto& c) {
   ...
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("min_rent_fee does not have a default value"),
                       configpowerup(make_config([&](auto& c) { c.min_rent_fee = {}; })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("min_rent_fee must be positive"),
                       configpowerup(make_config([&](auto& c) { c.min_rent_fee = asset::from_string("0.0000 TST"); })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("min_rent_fee must be positive"),
                       configpowerup(make_config([&](auto& c) { c.min_rent_fee = asset::from_string("-1.0000 TST"); })));

   // net assertions
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("current_weight_ratio is too large"),
                       configpowerup(make_config([](auto& c) { c.net.current_weight_ratio = powerup_frac + 1; })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("assumed_stake_weight/target_weight_ratio is too large"),
                       configpowerup(make_config([](auto& c) {
   ...
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("weight can't grow over time"),
                       configpowerup(make_config([](auto& c) { c.net.target_weight_ratio = powerup_frac + 1; })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("assumed_stake_weight does not have a default value"),
                       configpowerup(make_config([](auto& c) { c.net.assumed_stake_weight = {}; })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("assumed_stake_weight must be at least 1; a much larger value is recommended"),
                       configpowerup(make_config([](auto& c) { c.net.assumed_stake_weight = 0; })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("target_timestamp does not have a default value"),
                       configpowerup(make_config([&](auto& c) { c.net.target_timestamp = {}; })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("target_timestamp must be in the future"),
                       configpowerup(make_config([&](auto& c) { c.net.target_timestamp = control->head_block_time(); })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("target_timestamp must be in the future"), configpowerup(make_config([&](auto& c) {
                          c.net.target_timestamp = control->head_block_time() - fc::seconds(1);
                       })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("exponent must be >= 1"),
                       configpowerup(make_config([&](auto& c) { c.net.exponent = .999; })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("decay_secs must be >= 1"),
                       configpowerup(make_config([&](auto& c) { c.net.decay_secs = 0; })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("max_price does not have a default value"),
                       configpowerup(make_config([&](auto& c) { c.net.max_price = {}; })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("max_price doesn't match core symbol"), configpowerup(make_config([&](auto& c) {
                          c.net.max_price = asset::from_string("1000000.000 TST");
                       })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("max_price must be positive"),
                       configpowerup(make_config([&](auto& c) { c.net.max_price = asset::from_string("0.0000 TST"); })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("max_price must be positive"),
                       configpowerup(make_config([&](auto& c) { c.net.max_price = asset::from_string("-1.0000 TST"); })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("min_price doesn't match core symbol"), configpowerup(make_config([&](auto& c) {
                          c.net.min_price = asset::from_string("1000000.000 TST");
                       })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("min_price must be non-negative"),
                       configpowerup(make_config([&](auto& c) { c.net.min_price = asset::from_string("-1.0000 TST"); })));
   ...
   // cpu assertions
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("current_weight_ratio is too large"),
                       configpowerup(make_config([](auto& c) { c.cpu.current_weight_ratio = powerup_frac + 1; })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("assumed_stake_weight/target_weight_ratio is too large"),
                       configpowerup(make_config([](auto& c) {
   ...
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("weight can't grow over time"),
                       configpowerup(make_config([](auto& c) { c.cpu.target_weight_ratio = powerup_frac + 1; })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("assumed_stake_weight does not have a default value"),
                       configpowerup(make_config([](auto& c) { c.cpu.assumed_stake_weight = {}; })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("assumed_stake_weight must be at least 1; a much larger value is recommended"),
                       configpowerup(make_config([](auto& c) { c.cpu.assumed_stake_weight = 0; })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("target_timestamp does not have a default value"),
                       configpowerup(make_config([&](auto& c) { c.cpu.target_timestamp = {}; })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("target_timestamp must be in the future"),
                       configpowerup(make_config([&](auto& c) { c.cpu.target_timestamp = control->head_block_time(); })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("target_timestamp must be in the future"), configpowerup(make_config([&](auto& c) {
                          c.cpu.target_timestamp = control->head_block_time() - fc::seconds(1);
                       })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("exponent must be >= 1"),
                       configpowerup(make_config([&](auto& c) { c.cpu.exponent = .999; })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("decay_secs must be >= 1"),
                       configpowerup(make_config([&](auto& c) { c.cpu.decay_secs = 0; })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("max_price does not have a default value"),
                       configpowerup(make_config([&](auto& c) { c.cpu.max_price = {}; })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("max_price doesn't match core symbol"), configpowerup(make_config([&](auto& c) {
                          c.cpu.max_price = asset::from_string("1000000.000 TST");
                       })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("max_price must be positive"),
                       configpowerup(make_config([&](auto& c) { c.cpu.max_price = asset::from_string("0.0000 TST"); })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("max_price must be positive"),
                       configpowerup(make_config([&](auto& c) { c.cpu.max_price = asset::from_string("-1.0000 TST"); })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("min_price doesn't match core symbol"), configpowerup(make_config([&](auto& c) {
                          c.cpu.min_price = asset::from_string("1000000.000 TST");
                       })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("min_price must be non-negative"),
                       configpowerup(make_config([&](auto& c) { c.cpu.min_price = asset::from_string("-1.0000 TST"); })));
   BOOST_REQUIRE_EQUAL(wasm_assert_msg("min_price cannot exceed max_price"),
                       configpowerup(make_config([&](auto& c) {
...
BOOST_FIXTURE_TEST_CASE(weight_tests, powerup_tester) try {
   ...
   BOOST_REQUIRE_EQUAL("", configpowerup(make_config([&](powerup_config& config) {
   ...
   for (int i = 0; i <= 6; ++i) {
      if (i == 2) {
         ...
         BOOST_REQUIRE_EQUAL("", configpowerup({}));
      } else if (i) {
         produce_block(fc::days(1) - fc::milliseconds(500));
   ...
   // Extend transition time
   {
      int i = 7;
      produce_block(fc::days(1) - fc::milliseconds(500));
      BOOST_REQUIRE_EQUAL("", configpowerup(make_default_config([&](powerup_config& config) {
...
BOOST_AUTO_TEST_CASE(rent_tests) try {
   {
      ...
      BOOST_REQUIRE_EQUAL("", t.configpowerup(t.make_config([&](auto& config) {
         config.net.current_weight_ratio = powerup_frac;
      ...
      BOOST_REQUIRE_EQUAL("", t.configpowerup(t.make_default_config([&](auto& config) {
         // weight = stake_weight
   ...
   // net:100%, cpu:100%
   {
      ...
      BOOST_REQUIRE_EQUAL( //
            ...
            t.configpowerup(t.make_default_config([&](auto& config) {
